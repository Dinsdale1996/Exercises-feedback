...
#image detection
for i in range(1, 60):
    pathz = 'major_project/data/squat/squat_trainer'
    if (os.path.exists(pathz+str(i)+'.jpg')):
        #read image
        image = tf.io.read_file(pathz+str(i)+'.jpg')
        image = tf.image.decode_jpeg(pathz+str(i)+'.jpg')
        ...
        shaped = np.squeeze(np.multiply(keypoints_with_scores, [480,640,1]))
